...
namespace :approvals do
  ...
  task process_staff_report: :environment do
    ...
    Department.all.each do |department|
      ...
      StaffProfile.where(department:).order(:surname).each do |profile|
        puts "      #{profile}"
      end
    end
  end
  ...
  task :make_delegate, %i[delegate_uid delegator_uid] => [:environment] do |_t, args|
    ...
    begin
      Delegate.create!(delegate:, delegator:)
  ...
  namespace :reports do
    ...
    task :approved_trips, %i[start_date end_date file_path] => [:environment] do |_t, args|
      file_path = args[:file_path] || "./tmp/approved_request_report.csv"
      run_report(args:, file_path:, approved_only: true)
    end
    desc "Create a CSV report of all trips, regardless of status, within a date range"
    task :all_trips, %i[start_date end_date file_path] => [:environment] do |_t, args|
      file_path = args[:file_path] || "./tmp/all_request_report.csv"
      run_report(args:, file_path:)
    end
  end

  def make_requests(staff_profile:)
    1.upto(Random.rand(5...20)) do
      status = %w[pending approved denied changes_requested canceled].sample
      RandomRequestGenerator.generate_travel_request(creator: staff_profile, status:)
    end
    1.upto(Random.rand(6...24)) do
      status = %w[pending approved denied canceled recorded].sample
      RandomRequestGenerator.generate_absence_request(creator: staff_profile, status:)
    end
  end
end
