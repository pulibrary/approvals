...
RSpec.describe AbsentStaff, type: :model do
  ...
  describe "#list" do
    it "returns absent staff" do
      expect(described_class.list(start_date: Time.zone.now, end_date: 2.days.from_now,
                                  supervisor:)).to contain_exactly(mary, jack, jill,
                                                                   piper)
    end

    it "returns absent staff when window is inside the staff absence" do
      expect(described_class.list(start_date: 2.days.ago, end_date: 1.day.ago,
                                  supervisor:)).to contain_exactly(jill, piper)
    end

    it "returns absent staff when window is around end date of the absence" do
      expect(described_class.list(start_date: 2.days.from_now, end_date: 5.days.from_now,
                                  supervisor:)).to contain_exactly(mary, jack)
    end

    it "returns no one when the supervisor is other" do
      expect(described_class.list(start_date: 2.days.from_now, end_date: 5.days.from_now,
                                  supervisor: piper)).to be_empty
    end
  end
end
