...
RSpec.describe ReportingRequestList, type: :model do
  ...
  describe "GET #list_requests" do
    ...
    it "returns a success response" do
      list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: nil, search_query: nil, order: nil)
    ...
    it "accepts limit by status" do
      ...
      list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: { status: "approved" }, search_query: nil, order: nil)
      expect(list.map(&:id)).to contain_exactly(approved_absence.id)
    end

    it "accepts limit by request type absence" do
      list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: { "request_type" => "absence" }, search_query: nil, order: nil)
      expect(list.map(&:id)).to contain_exactly(*[other_absence, my_absence].map(&:id))
    end
    ...
    it "accepts limit by request type sick" do
      ...
      list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: { "request_type" => "sick" }, search_query: nil, order: nil)
      expect(list.map(&:id)).to contain_exactly(my_sick_absence.id)
    end

    it "accepts limit by request type travel" do
      list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: { "request_type" => "travel" }, search_query: nil, order: nil)
    ...
    it "accepts limit by request type business" do
      ...
      list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: { "request_type" => "business" }, search_query: nil, order: nil)
    ...
    it "accepts limit by status and request type" do
      ...
      list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: { "request_type" => "business", "status" => "approved" }, search_query: nil, order: nil)
      expect(list.map(&:id)).to contain_exactly(my_business_travel.id)
    end

    it "accepts limit by department" do
      list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: { "department" => staff_profile.department.number }, search_query: nil, order: nil)
  ...
  describe "GET #my_requests with sort params" do
    ...
    context "sort by start date" do
      it "by default sorts by start date descending" do
        list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: nil, search_query: nil, order: "start_date_desc")
        expect(list.map(&:id)).to eq [r2, r1, r3].map(&:id)
      end

      it "sorts ascending" do
        list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: nil, search_query: nil, order: "start_date_asc")
    ...
    context "sort by date created" do
      it "sorts ascending" do
        list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: nil, search_query: nil, order: "created_at_asc")
        expect(list.map(&:id)).to eq [r1, r2, r3].map(&:id)
      end

      it "sorts descending" do
        list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: nil, search_query: nil, order: "created_at_desc")
    ...
    context "sort by date modified" do
      it "sorts ascending" do
        list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: nil, search_query: nil, order: "updated_at_asc")
        expect(list.map(&:id)).to eq [r2, r3, r1].map(&:id)
      end

      it "sorts descending" do
        list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: nil, search_query: nil, order: "updated_at_desc")
        expect(list.map(&:id)).to eq [r1, r3, r2].map(&:id)
      end
    end
  end
  ...
  describe "GET #my_requests with searching" do
    it "retrieves a result" do
      ...
      list = ReportingRequestList.list_requests(creator: staff_profile, request_filters: { "status" => "approved" }, search_query: "balloons", order: nil)
