...
RSpec.describe RecordRequestList, type: :model do
  ...
  let(:other_absence) { FactoryBot.create(:absence_request, action: :approve) }
  let(:other_travel) { FactoryBot.create(:travel_request) }
  let(:my_absence) { FactoryBot.create(:absence_request, creator: staff_profile, start_date: Time.zone.tomorrow, action: :approve) }
  let(:my_pending_absence) { FactoryBot.create(:absence_request, creator: staff_profile, start_date: Time.zone.tomorrow) }
  ...
  describe "GET #list_requests" do
    ...
    it "returns a success response" do
      ...
      expect(list).to eq([my_absence])
    end

    it "different results for a different profile" do
      list = described_class.list_requests(current_staff_profile: other_absence.creator, request_filters: nil, search_query: nil, order: nil)
      expect(list).to eq([other_absence])
    end
    ...
    it "accepts limit by request type sick" do
      ...
      list = described_class.list_requests(current_staff_profile: staff_profile, request_filters: { "request_type" => "sick" }, search_query: nil, order: nil)
  ...
  describe "GET #list_requests with sort params" do
    ...
    context "sort by start date" do
      it "by default sorts by start date descending" do
        list = described_class.list_requests(current_staff_profile: staff_profile, request_filters: nil, search_query: nil, order: "start_date_desc")
        expect(list.map(&:id)).to eq [r2, r1, r3].map(&:id)
      end

      it "sorts ascending" do
        list = described_class.list_requests(current_staff_profile: staff_profile, request_filters: nil, search_query: nil, order: "start_date_asc")
    ...
    context "sort by date created" do
      it "sorts ascending" do
        list = described_class.list_requests(current_staff_profile: staff_profile, request_filters: nil, search_query: nil, order: "created_at_asc")
        expect(list.map(&:id)).to eq [r1, r2, r3].map(&:id)
      end

      it "sorts descending" do
        list = described_class.list_requests(current_staff_profile: staff_profile, request_filters: nil, search_query: nil, order: "created_at_desc")
    ...
    context "sort by date modified" do
      it "sorts ascending" do
        list = described_class.list_requests(current_staff_profile: staff_profile, request_filters: nil, search_query: nil, order: "updated_at_asc")
        expect(list.map(&:id)).to eq [r2, r3, r1].map(&:id)
      end

      it "sorts descending" do
        list = described_class.list_requests(current_staff_profile: staff_profile, request_filters: nil, search_query: nil, order: "updated_at_desc")
  ...
  describe "GET #my_requests with searching" do
    it "retrieves a result" do
      ...
      absence_request2 = FactoryBot.create(:absence_request, creator: staff_profile, absence_type: :sick, action: :approve)
      absence_request3 = FactoryBot.create(:absence_request, creator: staff_profile, absence_type: :sick, action: :approve)
      FactoryBot.create(:note, content: "elephants love balloons", request: absence_request)
      FactoryBot.create(:note, content: "cats also love balloons", request: absence_request2)
      FactoryBot.create(:note, content: "flamingoes are pink because of shrimp", request: absence_request3)

      list = described_class.list_requests(current_staff_profile: staff_profile, request_filters: { "absence_type" => "sick" }, search_query: "balloons", order: nil)
