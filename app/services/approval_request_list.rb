...
class ApprovalRequestList < RequestList
  class << self
    def list_requests(approver:, request_filters:, search_query:, order:, page: 0)
      ...
      filter_records_by_approver(records:, approver:, page:)
    end
    ...
    private
      ...
      def filter_records_by_approver(records:, approver:, page:)
        filtered = records.select do |request|
          only_next_supervisor(request:,
                               approver:) && !already_approved?(request:, approver:)
        end
      ...
      def only_next_supervisor(request:, approver:)
        ...
        return false if approver_index.blank?

        previous_supervisors = supervisors.slice(0, approver_index)
        previous_supervisors.all? { |supervisor| already_approved?(request:, approver: supervisor) }
      end

      def already_approved?(request:, approver:)
        StateChange.where(request_id: request.id, agent_id: approver.id, action: "approved").any?
      end
