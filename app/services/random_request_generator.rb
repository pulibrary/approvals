...
class RandomRequestGenerator
  class << self
    def generate_travel_request(creator:, status: "pending")
      ...
      request = TravelRequest.create!(creator:,
                                      event_requests_attributes: [event_requests_attributes(recurring_events:)],
    ...
    def generate_absence_request(creator:, status: "pending")
      ...
      request = AbsenceRequest.create!(creator:, start_date:, end_date:,
      ...
      def travel_state_change(request, supervisor_approved, action)
        ...
        if supervisor_approved && (request.status != "approved")
          request.aasm.fire(action,
                            agent: request.creator.department.head)
        end
        request.travel_category = TravelCategoryList.categories.sample
        request
      end
